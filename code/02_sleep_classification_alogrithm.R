...
calculate_threshold <- function(log_vedba_vector, thres_value = 0.7) {
  # Remove NA values
...
time_shift <- d1$local_timestamp - 9*60*60
...
start_date <- as.Date(min(d1$local_timestamp)- 9*60*60)
...
d1$night_date <- as.Date( d1$local_timestamp - 9*60*60 )
...
spec_nights <- sort(unique(d1$night_date))[300:302]
...
for( tag in tag_names){
  ...
  nights <- spec_nights # unique( id_dat$night_date )
  ...
  for(night in nights){
    ...
    sorted_times <- c( as.POSIXct( paste( as.Date( night, origin = "1970-01-01", tz = "EAT"), '18:00:00' ), tz = "EAT"), sort( night_dat$timestamp ), as.POSIXct( paste( as.Date( ( night + 1 ), origin = "1970-01-01", tz = "EAT"), '06:30:00' ), tz = "EAT") )
    ...
    if( sep_thresh ){
      ...
      if (sum(is.na(night_dat$pot_sleep)) > 100){
        print(paste(tag, night))
      }
      ## find the run length encoding of periods above and below the threshold
...
# check that all night exist
result <- full_dat %>%
  dplyr::group_by(tag, night_date) %>%
  dplyr::summarise(
    na_pot_sleep = sum(is.na(pot_sleep)),
  ) %>%
  dplyr::filter(night_date %in% spec_nights)
...
sleep_per$onset <- as.POSIXct( sleep_per$onset, origin = "1970-01-01 00:00:00", tz = "EAT" )

## reformat waking timestamp
sleep_per$waking <- as.POSIXct( sleep_per$waking, origin = "1970-01-01 00:00:00", tz = "EAT" )
