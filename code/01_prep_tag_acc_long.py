...
if matching_files:
    ...
    acc_data = pd.read_parquet(most_recent_file) 
...
acc_data = acc_data.rename(columns={
    ...
    'eobs_accelerations_raw': 'eobs_accelerations_raw_str'
})

# Select necessary columns and ensure timestamp is in datetime format
acc_data = acc_data[['animal_id', 'tag_id', 'timestamp', 'eobs_accelerations_raw_str']]
...
unique_animals = acc_data['animal_id'].unique()
...
for current_animal in tqdm(unique_animals, desc="Processing animals"):
    ...
    animal_acc_data = acc_data[acc_data['animal_id'] == current_animal].copy()
    
    # Get all tags for this animal
    animal_tags = animal_acc_data['tag_id'].unique()
    ...
    for current_tag in animal_tags:
        ...
        individual_acc_data = animal_acc_data[animal_acc_data['tag_id'] == current_tag].copy()
...
# acc_folder = "/mnt/EAS_shared/baboon/working/data/raw/2025/acc/"
# # pattern = os.path.join(acc_folder, "MBRP_acc_*.csv")
# pattern = os.path.join(acc_folder, "Baboons MBRP Mpala Kenya*.csv")
...
#     most_recent_file = max(matching_files, key=os.path.getmtime)
#     print(f"Reading most recent file: {most_recent_file}")
#     old_acc = pd.read_csv(most_recent_file)
# else:
