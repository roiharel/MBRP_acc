...
for (ind in  1:length(inactivity_files)) { #
  ...
  for (n in 1:length(nights)) {
    ...
    sleep_per[sleep_per$tag == tag_id & sleep_per$night_date == night, ]$n_bursts <- unique(night_dat$n_bursts)
    
    sleep_per[sleep_per$tag == tag_id & sleep_per$night_date == night, ]$max_time_diff <- unique(night_dat$max_time_diff)
    ...
    if (nrow(SPT_dat) > 0) {
      ...
      if (TST > 1500 | SPT > 1500) plot(night_dat$timestamp, night_dat$logvedba)
...
write.csv(all_sleep_per, file = "results/combined_sleep_analysis.csv", row.names = FALSE)
